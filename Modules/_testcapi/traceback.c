...
#include "traceback.h"  // PyUnstable_CollectTraceback, PyUnstable_PrintTraceback


/* collect_traceback([max_frames]) -> list of (filename, lineno, name) | None
 *
 * Calls PyUnstable_CollectTraceback() on the current tstate and returns the
 * collected frames as a list of (filename, lineno, name) tuples where lineno
 * is an int or None if unknown.  Returns None if the tstate has no Python
 * frame (i.e. PyUnstable_CollectTraceback() returned -1). */
static PyObject *
traceback_collect(PyObject *self, PyObject *args)
{
    ...
    int n = PyUnstable_CollectTraceback(tstate, frames, max_frames);
    ...
    for (int i = 0; i < n; i++) {
        ...
        PyObject *item = Py_BuildValue("(sNs)",
...
/* print_traceback(fd, [(filename, lineno, name), ...][, write_header]) -> None
 *
 * Constructs a PyUnstable_FrameInfo array from a Python list of tuples and
 * calls PyUnstable_PrintTraceback().  Used to test the print format in
...
traceback_print(PyObject *self, PyObject *args)
{
    ...
    for (Py_ssize_t i = 0; i < n; i++) {
        ...
        int lineno;
        if (!PyArg_ParseTuple(item, "sis", &filename, &lineno, &name)) {
            PyMem_Free(frames);
            return NULL;
        }
    ...
    PyUnstable_PrintTraceback(fd, frames, (int)n, write_header);
    PyMem_Free(frames);
    Py_RETURN_NONE;
}


/* collect_and_print_traceback(fd[, max_frames[, write_header]]) -> None
 *
 * Calls PyUnstable_CollectTraceback() + PyUnstable_PrintTraceback() in one
...
traceback_collect_and_print(PyObject *self, PyObject *args)
{
    ...
    int n = PyUnstable_CollectTraceback(tstate, frames, max_frames);
    if (n >= 0) {
        PyUnstable_PrintTraceback(fd, frames, n, write_header);
    }
    PyMem_Free(frames);
    Py_RETURN_NONE;
}
...
static PyMethodDef traceback_methods[] = {
    {"collect_traceback",         traceback_collect,         METH_VARARGS},
    {"print_traceback",           traceback_print,           METH_VARARGS},
    {"collect_and_print_traceback", traceback_collect_and_print, METH_VARARGS},
    {NULL},
};
